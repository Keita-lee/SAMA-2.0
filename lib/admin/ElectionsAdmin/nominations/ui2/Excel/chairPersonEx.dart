...
class ChairPersonEx {
  exportChairPerson(startDate, endDate, chairList, branch) {
    ...
    var hdiCell = sheet.cell(CellIndex.indexByString("D5"));
    hdiCell.value = TextCellValue("Hdi");
    hdiCell.cellStyle = cellBold;
    ...
    for (int i = 0; i < chairList.length; i++) {
      ...
      cell8.value = TextCellValue(chairList[i]['name']);

      var cell9 = sheet.cell(CellIndex.indexByString("B${i + 7}"));
      cell9.value = TextCellValue(chairList[i]['SamaNr']);

      var cell10 = sheet.cell(CellIndex.indexByString("C${i + 7}"));
      cell10.value = TextCellValue(chairList[i]['hpca']);

      var cell12 = sheet.cell(CellIndex.indexByString("D${i + 7}"));
      cell12.value = TextCellValue(chairList[i]['hdiStatus']);
